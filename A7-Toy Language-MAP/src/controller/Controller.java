...
public class Controller {
    ...
    public void allStep() throws InterruptedException {
        ...
        while(prgList.size() > 0){
            prgList.forEach(prg -> prg.getHeap().setContent(safeGarbageCollector(getAddrFromSymTable(prg.getSymTable().getContent().values()), getAddrFromHeapTable(prg.getHeap().getContent().values()), prg)));
